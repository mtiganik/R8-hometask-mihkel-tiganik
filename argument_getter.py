...
def getArgs(argv):
    opts, args = getopt.getopt(argv,"hlp:t:",["help","page=", "per_page=","no_comment", "list"])
    res = {'page':DEFAULT_PAGE, 'per_page':DEFAULT_PER_PAGE, 'no_comment':DEFAULT_COMMENTS_COUNT_DISABLED, 'list': DEFAUlt_SHOW_IN_LIST}
    for opt, arg in opts:
        if opt in ('-h','--help'):
            generateHelp()
            sys.exit()
            ...
            res['list'] = True
        elif opt in ("--no_comment"):
            res['no_comment'] = True
        elif opt in ('-p', '--per_page'):
            res['page'] = getInt(arg)
